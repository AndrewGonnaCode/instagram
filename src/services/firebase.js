...
export async function updateFollowedUserFollowers(
  ...
  loggedInUserId, // the user that karl requests to follow
  isFollowingProfile // true/false (am i currently following this person?)
) {
  return firebase
    .firestore()
...
export async function getProfilePhoto(profileUserId, userId, photoDocId){

  const result =  await getUserPhotosByUserId(profileUserId)

     const [chosen] = result.filter((photo)=>photo.docId === photoDocId)
     let userLikedPhoto = false;
   
     if(chosen.likes.includes(userId)){
       userLikedPhoto = true
     }

     return userLikedPhoto
...
export async function getUserPhotosByUserId(userId, loggedInUserId) {
  ...
  const photosWithLike = await Promise.all(
